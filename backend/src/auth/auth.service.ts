...
export class AuthService {
  ...
  async register(input: { firstName: string; lastName: string; email: string; password: string }, request: Request) {
    ...
    const user = await this.db.user.create({ data: { firstName: input.firstName, lastName: input.lastName, email: input.email, normalizedEmail, passwordHash: await argon2.hash(input.password) } });
  ...
  async login(input: { email: string; password: string }, request: Request) {
    ...
    if (!user || user.status !== 'ACTIVE' || !(await argon2.verify(user.passwordHash, input.password))) throw new UnauthorizedException('Invalid email or password.');
